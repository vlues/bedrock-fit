...
echo "Nothing sensitive lives in the repo itself: there's no API key stored"
echo "anywhere client-side — Claude features route through the optional"
echo "backend below, gated by sign-in, not a key in the browser."
echo ""
echo "Next: open the URL above on each phone, run onboarding once, and"
echo "'Add to Home Screen' so it behaves like an app."
echo ""
echo "Want cross-device sync + Claude features (insight, chat, photo"
echo "feedback, Fitbit breakdowns)? Run cloudflare-worker/deploy-backend.sh"
echo "once, then cloudflare-worker/create-account.sh for each person — see"
echo "README.md 'Accounts, sync & Claude' for details."
echo "──────────────────────────────────────────────────────────"
